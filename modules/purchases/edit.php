...
if ($purchase_id <= 0) {
    set_flash_message('danger', 'Invalid purchase parameter.');
    header("Location: index.php");
    exit();
}

// Fetch purchase and details
try {
    ...
    if (!$purchase) {
        set_flash_message('danger', 'Purchase record not found.');
        header("Location: index.php");
        exit();
    }

    // Fetch purchase line items
...
// Process Purchase Edit update POST
    ...
    if (!verify_csrf_token($csrf_token)) {
        ...
        if ($supplier_id <= 0 || empty($purchase_date) || empty($item_products)) {
            ...
            try {
                ...
                // 1. Revert OLD stock quantities (subtract from current_stock because purchase adds stock)
                $revert_stmt = $pdo->prepare("UPDATE products SET current_stock = current_stock - ? WHERE id = ?");
                foreach ($purchase_items as $old_item) {
                    $revert_stmt->execute([$old_item['quantity'], $old_item['product_id']]);
                }

                // 2. Calculate new total amount and compile lines
                $total_amount = 0;
                $lines = [];
                ...
                $del_items_stmt = $pdo->prepare("DELETE FROM purchase_items WHERE purchase_id = ?");
                $del_items_stmt->execute([$purchase_id]);

                // 4. Insert new items and add stock quantities
                $ins_item = $pdo->prepare("
                    INSERT INTO purchase_items (purchase_id, product_id, quantity, rate, amount) 
                    VALUES (?, ?, ?, ?, ?)
                ");
                $add_stock = $pdo->prepare("
                    UPDATE products 
                ...
                // 5. Update main purchase log details
                $upd_stmt = $pdo->prepare("
                    UPDATE purchases 
                    SET supplier_id = ?, invoice_no = ?, purchase_date = ?, total_amount = ?
                    WHERE id = ?
                ");
                $upd_stmt->execute([
                    $supplier_id,
                ...
                log_activity($pdo, "Updated purchase receipt (ID $purchase_id) from supplier ID $supplier_id. New Total: ₹" . number_format($total_amount, 2));

                $pdo->commit();

                set_flash_message('success', "Purchase transaction updated successfully.");
                header("Location: index.php");
                exit();
...
<div class="row mb-4">
    <div class="col-12 d-flex justify-content-between align-items-center">
        <div>
            <h3 class="fw-bold" style="color: var(--navy-dark);"><i class="fa-solid fa-pen-to-square me-2 text-warning"></i>Edit Purchase Receipt</h3>
            <p class="text-muted" style="font-size: 0.9rem;">Modify receipt parameters and update product stock changes.</p>
        </div>
        <div>
...
<form action="edit.php?id=<?php echo $purchase_id; ?>" method="POST" id="purchaseForm">
    <?php echo csrf_input(); ?>
        ...
        <div class="col-12 col-lg-4">
            <div class="card card-premium shadow-sm">
                <div class="card-premium-header">
                    <h5 class="card-premium-title"><i class="fa-solid fa-file-invoice me-2 text-warning"></i>Invoice Information</h5>
                </div>
                <div class="card-premium-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control form-control-premium" id="invoice_no" name="invoice_no" placeholder="e.g. BILL-9872" value="<?php echo htmlspecialchars($purchase['invoice_no']); ?>">
                    </div>

                    <div class="mb-3">
                        <label for="purchase_date" class="form-label fw-semibold text-muted" style="font-size: 0.8rem;">BILLING DATE *</label>
                        <input type="date" class="form-control form-control-premium" id="purchase_date" name="purchase_date" required value="<?php echo htmlspecialchars($purchase['purchase_date']); ?>">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-12 col-lg-8">
            <div class="card card-premium shadow-sm">
                <div class="card-premium-header d-flex justify-content-between align-items-center">
                    <h5 class="card-premium-title"><i class="fa-solid fa-list-check me-2 text-warning"></i>Invoice Items</h5>
                ...
                <div class="card-premium-body">
                    <div class="table-responsive">
                        <table class="table align-middle" id="itemsTable">
                            ...
                            <tbody id="line-items-tbody">
                                <!-- Existing Rows will be inserted by PHP -->
                                <?php foreach ($purchase_items as $index => $item): ?>
                                    <tr id="row-edit-<?php echo $item['id']; ?>">
                                        <td>
                                            <select class="form-select form-control-premium prod-select" name="prod_id[]" required>
                                                <option value="">-- Choose --</option>
                                                <?php foreach ($products as $p): ?>
                                                    <option value="<?php echo $p['id']; ?>" data-price="<?php echo $p['purchase_price']; ?>" <?php echo ($item['product_id'] == $p['id']) ? 'selected' : ''; ?>><?php echo htmlspecialchars($p['name']); ?> (SKU: <?php echo htmlspecialchars($p['sku']); ?>)</option>
                                                <?php endforeach; ?>
                                            </select>
                                        </td>
                                        <td>
                                            <input type="number" min="1" class="form-control form-control-premium qty-input" name="qty[]" required placeholder="0" value="<?php echo $item['quantity']; ?>">
                                        </td>
                                        <td>
                                            <input type="number" step="0.01" min="0" class="form-control form-control-premium rate-input" name="rate[]" required placeholder="0.00" value="<?php echo $item['rate']; ?>">
                                        </td>
                                        <td>
                                            <input type="text" class="form-control form-control-premium bg-light amount-input" readonly placeholder="0.00" value="<?php echo number_format($item['amount'], 2, '.', ''); ?>">
                                        </td>
                    ...
                    <div class="border-top pt-3 d-flex justify-content-end align-items-center">
                        <div class="text-end">
                            ...
                            <span class="fs-4 fw-bold text-success" id="grandTotalDisplay">₹<?php echo number_format($purchase['total_amount'], 2); ?></span>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="d-flex gap-2 mb-5">
        <button type="submit" class="btn btn-accent px-4 py-2">Update Receipt</button>
